...
if arch == CPU() # If there's no CPU (e.g. if we wanna test shit on a laptop) let's use a smaller number of points!
    params = (; params..., Nx = 80, Ny = 30, Nz = 40)
end
...
params = (; params...,
          ...
          u₁_west = 62/(200*2), #u₁_west, discharge/(DY*DZ)          
...
if mass_flux
    @inline sponge_u(x, y, z, t, u, p) = -west_mask(x, y, z, t, p) / p.σ * (u - p.u₁_west)-east_mask(x, y, z, p) / p.σ/10 * (u - p.u₁_west*200*2/1200/25) # Nudges east to DYout*DZout/DYDZeast 
...
@inline sponge_T(x, y, z, t, T, p) = -west_mask(x, y, z, t, p) / p.σ * (T - 0)-east_mask(x, y, z, p) / p.σ/10 * (T - T∞(z, p)) # nudges T to T∞
@inline sponge_S(x, y, z, t, S, p) = -west_mask(x, y, z, t, p) / p.σ * (S - 0)-east_mask(x, y, z, p) / p.σ/10 * (S - S∞(z, p)) # nudges S to S∞
@inline sponge_c(x, y, z, t, c, p) = -west_mask(x, y, z, t, p) / p.σ * (c - 1)-east_mask(x, y, z, p) / p.σ/10 * (c - 0) # nudges S to S∞
@inline sponge_d(x, y, z, t, d, p) = -west_mask(x, y, z, t, p) / p.σ * (d - 0)-east_mask(x, y, z, p) / p.σ/100 * (d - d∞(z, p)) # nudges S to S∞
#----
...
simulation = Simulation(model, Δt=Δt₀,
                        stop_time = 24000seconds, # when to stop the simulation
)
...
simulation.output_writers[:fields] = NetCDFOutputWriter(model, outputs, 
                                                        schedule = TimeInterval(1000seconds),
...
simulation.output_writers[:surface_slice_writer] =
    NetCDFOutputWriter(model, (; u,v,w, T,S, c, d), filename="face3.nc",
                       schedule=TimeInterval(120), indices=(3, :, :),
                        overwrite_existing = overwrite_existing)

simulation.output_writers[:surface_slice_writer2] =
    NetCDFOutputWriter(model, (; u,v,w, T,S, c, d), filename="face1.nc",
                       schedule=TimeInterval(120), indices=(1, :, :),
                        overwrite_existing = overwrite_existing)


simulation.output_writers[:surface_slice_writer1] =
    NetCDFOutputWriter(model, (; u,v,w, T,S,c, d), filename="top.nc",
                       schedule=TimeInterval(120), indices=(:, :, Nz),
                        overwrite_existing = overwrite_existing)
#simulation.output_writers[:surface_slice_writer2] =
...
simulation.output_writers[:y_slice_writer] =
    NetCDFOutputWriter(model,(; u, v, w, T,S,c,d ), filename="midy.nc",
                       schedule=TimeInterval(120), indices=(:, 250, :), 
                       overwrite_existing = overwrite_existing)


output_interval=300seconds
